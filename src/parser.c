...
static const char *get_str_value_from_json(json_object *jso, const char *key)
{
	...
	str_value = json_object_get_string(jobjkey);
		...
		create_device_item_cb cb)
{
	...
	id = get_str_value_from_json(array_item, KNOT_JSON_FIELD_DEVICE_ID);
	if (!id)
		return NULL;
